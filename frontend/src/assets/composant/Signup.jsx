...
import { FaEye, FaEyeSlash, FaCheck, FaShoppingBag, FaArrowRight, FaSpinner, FaGoogle, FaFacebookF, FaApple } from 'react-icons/fa';

const Signup = () => {
  const navigate = useNavigate();
  const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID;
  const facebookAppId = import.meta.env.VITE_FACEBOOK_APP_ID; // Ajout de l'ID de l'app Facebook
  const appleClientId = import.meta.env.VITE_APPLE_CLIENT_ID; // Ajout de l'ID de l'app Apple
  ...
  useEffect(() => {
    const loadFacebookSDK = () => {
      ...
      script.onload = () => {
        window.FB.init({
          appId: facebookAppId,
          cookie: false, // Pas de cookies pour JWT
          xfbml: true,
          version: 'v20.0',
        });
      };
  ...
  // Charger le SDK Apple (Sign in with Apple)
  useEffect(() => {
    const loadAppleSDK = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        is_seller_pending: formData.is_seller,
      };
  ...
  const handleFacebookSuccess = async () => {
    if (!window.FB) {
      toast.error('SDK Facebook non chargé');
      return;
    }
    try {
      ...
      window.FB.login(
        async (response) => {
          if (response.status === 'connected') {
            ...
            try {
              const res = await axios.post(
                'http://localhost:8000/api/users/facebook/login/',
              ...
              console.error('Erreur Facebook Sign-In:', err);
              const errorMessage =
                err.response?.data?.error ||
                err.response?.data?.detail ||
  ...
  const handleAppleSuccess = async () => {
    if (!window.AppleID) {
      toast.error('SDK Apple non chargé');
      return;
    }
    try {
      const response = await window.AppleID.auth.signIn();
      const { id_token, user } = response.authorization;
      const payload = { id_token };
      if (user && user.name) {
        payload.first_name = user.name.firstName || '';
        payload.last_name = user.name.lastName || '';
      }
      const res = await axios.post(
        'http://localhost:8000/api/users/apple/login/',
        payload,
        {
          headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
          withCredentials: false,
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-purple-700 text-white p-8 md:w-1/2 flex flex-col justify-center items-center text-center relative overflow-hidden">
          ...
          <div className="relative z-10 mb-8">
            ...
            <p className="opacity-90 max-w-md">
              Créez votre compte et bénéficiez de 10% de réduction sur votre première commande + un cadeau surprise !
            </p>
          </div>
          ...
          <p className="relative z-10 mt-6 text-sm opacity-80">
            Déjà membre?{' '}
            <Link to="/login" className="font-semibold underline hover:opacity-90 transition">
              Connectez-vous
            </Link>
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="flex space-x-4">
              <div className="w-1/2 relative">
                ...
                <div
                  className={`absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none transition duration-200 ${
                    first_name ? 'opacity-100' : 'opacity-0'
                  }`}
                >
              ...
              <div className="w-1/2 relative">
                ...
                <div
                  className={`absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none transition duration-200 ${
                    last_name ? 'opacity-100' : 'opacity-0'
                  }`}
                >
                  <FaCheck className="text-green-500" />
                </div>
            ...
            <div className="relative">
              ...
              <input
                ...
                } rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-200 transition duration-200`}
              ...
              <div className="mt-2">
                <div className="flex mb-1">
                  <div className="w-full bg-gray-200 rounded-full h-1">
                    <div className={`h-full rounded-full ${passwordStrength.color}`} style={{ width: passwordStrength.width }}></div>
                  </div>
                </div>
            ...
            <div className="relative">
              ...
              {fieldErrors.password2 && <p className="mt-1 text-sm text-red-600">{fieldErrors.password2}</p>}
              <div
                className={`absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none transition duration-200 ${
                  password2 && password === password2 ? 'opacity-100' : 'opacity-0'
                }`}
              >
                <FaCheck className="text-green-500" />
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="social-btn flex items-center justify-center h-[42px]">
                <GoogleLogin
              ...
              <button
                type="button"
                className="social-btn flex items-center justify-center py-2 px-4 border border-gray-300 rounded-lg hover:bg-gray-50 transition transform hover:-translate-y-0.5 active:translate-y-0 h-[42px]"
