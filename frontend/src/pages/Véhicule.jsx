...
import { 
  ...
  Car, Battery, Star, Heart, Search, ChevronRight, 
  BusFront, CarFront, ToolCase, Fence, Camera, Ship,
  Ruler, Bath,
  Bike,
  BikeIcon
} from 'lucide-react';
...
const Véhicule = () => {
  ...
  // Récupérer la catégorie Véhicule et ses sous-catégories
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        const [categoryRes, propertiesRes] = await Promise.all([
          axios.get('http://localhost:8000/api/categories/categories/Véhicule/'),
          axios.get('http://localhost:8000/api/listings/?category=Véhicule'),
        ...
        const transformedProperties = propertiesRes.data.results.map(item => ({
          ...
          type: item.category_name ? item.subcategory.name : 'Véhicule',
  ...
  const vehiculesCategories = subcategories.map(sub => ({
    name: sub.name,
  ...
  function getIconForSubcategory(name) {
    const icons = {
      'Voitures': Car,
      'Motos & Scooters': BikeIcon,
      'Location de véhicules': CarFront,
      'Accessoires & pièces détachées': ToolCase,
      'Camions & Bus': BusFront,
  ...
  function getColorForSubcategory(name) {
    const colors = {
      ...
      'Motos & Scooters': 'bg-purple-100 text-purple-600',
      'Location de véhicules': 'bg-green-100 text-green-600',
      'Camions & Bus': 'bg-indigo-100 text-indigo-600',
      'Accessoires & pièces détachées': 'bg-orange-100 text-orange-600',
      'Bateaux': 'bg-pink-100 text-pink-600',
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 mb-8">
          ...
          <span className="text-gray-700 font-medium">{category?.name || 'Véhicule'}</span>
        </div>
        ...
        <div className="relative bg-gradient-to-r from-blue-600 to-blue-800 rounded-xl p-8 mb-12 text-white overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl mx-auto text-center">
            ...
            <h1 className="text-4xl font-bold mb-4 font-serif">{category?.name || 'Véhicule'}</h1>
            <p className="text-lg text-blue-100 mb-6">
              {category?.description || 'Découvrez notre sélection exclusive de Véhicule'}
            </p>
        ...
        {/* Categories Grid */}
        {vehiculesCategories.length > 0 && (
          <section className="mb-16">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-2xl font-bold text-gray-800 font-serif">Nos catégories Véhicules</h2>
              <button 
                onClick={() => navigate('/vehicules/toutes-categories')}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-6">
              {vehiculesCategories.map((category) => {
                const IconComponent = category.icon;
                return (
                  <div 
                    key={category.name}
                    onClick={() => navigate(`/vehicules/categorie/${category.name.toLowerCase()}`)}
                    className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 cursor-pointer"
                  >
        ...
        <section className="mb-16">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-bold text-gray-800 font-serif">
              {selectedType === "Tous" ? "Tous nos Véhicules" : selectedType}
            </h2>
            <button 
              onClick={() => navigate('/vehicules/toutes-les-annonces')}
              className="text-blue-600 hover:text-blue-800 flex items-center"
            >
              Tout voir <ChevronRight className="h-4 w-4 ml-1" />
            </button>
