...
import { 
  ArrowLeft, Home, Building, Building2, Trees, MapPin, Bed, 
  Warehouse, Store, Star, Heart, Search, ChevronRight, 
  ChevronLeft, Castle, School, Fence, Users, Calendar,
  Ruler, Bath
} from 'lucide-react';
...
const Immobilier = () => {
  ...
  // Récupérer la catégorie Immobilier et ses sous-catégories
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        const [categoryRes, propertiesRes] = await Promise.all([
          axios.get('http://localhost:8000/api/categories/categories/Immobilier/'),
          axios.get('http://localhost:8000/api/listings/?category=Immobilier')
        ]);
        ...
        const transformedProperties = propertiesRes.data.results.map(item => ({
          ...
          type: item.subcategory || 'Immobilier',
  ...
  const immobilierCategories = subcategories.map(sub => ({
    name: sub.name,
  ...
  function getIconForSubcategory(name) {
    const icons = {
      'Villas': Home,
      'Appartements': Building,
      'Terrains': Trees,
      'Immeubles': Building2,
      'Bureaux': Store,
      'Chambres': Bed,
      'Appartements meublés': Warehouse,
      'Bureaux & Commerces':Castle,
    };
  ...
  function getColorForSubcategory(name) {
    const colors = {
      'Villas': 'bg-blue-100 text-blue-600',
      'Appartements': 'bg-purple-100 text-purple-600',
      'Terrains': 'bg-green-100 text-green-600',
      'Immeubles': 'bg-indigo-100 text-indigo-600',
      'Bureaux': 'bg-orange-100 text-orange-600',
      'Chambres': 'bg-pink-100 text-pink-600',
      'Appartements meublés': 'bg-red-100 text-red-600',
      'Bureaux & Commerces': 'bg-yellow-100 text-yellow-600',
    };
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 mb-8">
          ...
          <span className="text-gray-700 font-medium">{category?.name || 'Immobilier'}</span>
        </div>
        ...
        <div className="relative bg-gradient-to-r from-blue-600 to-blue-800 rounded-xl p-8 mb-12 text-white overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl mx-auto text-center">
            ...
            <h1 className="text-4xl font-bold mb-4 font-serif">{category?.name || 'Immobilier'}</h1>
            <p className="text-lg text-blue-100 mb-6">
              {category?.description || 'Découvrez notre sélection exclusive de biens immobiliers'}
            </p>
        ...
        {/* Categories Grid */}
        {immobilierCategories.length > 0 && (
          <section className="mb-16">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-2xl font-bold text-gray-800 font-serif">Nos catégories immobilières</h2>
              <button 
                onClick={() => navigate('/immobilier/toutes-categories')}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-6">
              {immobilierCategories.map((category) => {
                const IconComponent = category.icon;
                return (
                  <div 
                    key={category.name}
                    onClick={() => navigate(`/immobilier/categorie/${category.name.toLowerCase()}`)}
                    className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 cursor-pointer"
                  >
        ...
        <section className="mb-16">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-bold text-gray-800 font-serif">
              {selectedType === "Tous" ? "Tous nos biens immobiliers" : selectedType}
            </h2>
            <button 
              onClick={() => navigate('/immobilier/toutes-les-annonces')}
              className="text-blue-600 hover:text-blue-800 flex items-center"
            >
              Tout voir <ChevronRight className="h-4 w-4 ml-1" />
            </button>
        ...
        <section className="bg-gradient-to-r from-blue-600 to-blue-800 rounded-xl p-8 text-center text-white mb-12">
          ...
          <button 
            ...
            className="border border-white text-white hover:bg-white hover:text-blue-600 px-4 py-2  rounded-md text-base sm:text-lgfont-medium transition-colors"
          >
            Publier une annonce
          </button>
        </section>
