...
import { FaCompactDisc, FaBicycle,FaDrum,FaTruckPlane,FaFootball  } from "react-icons/fa6";
...
const Sports = () => {
  ...
  // Récupérer la catégorie Sports & Loisirs et ses sous-catégories
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        const [categoryRes, propertiesRes] = await Promise.all([
          axios.get('http://localhost:8000/api/categories/categories/Sports & Loisirs/'),
          axios.get('http://localhost:8000/api/listings/?category=Sports & Loisirs'),
        ...
        const transformedProperties = propertiesRes.data.results.map(item => ({
          ...
          type: item.category_name ? item.subcategory.name : 'Sports & Loisirs',
  ...
  const sportsCategories = subcategories.map(sub => ({
    name: sub.name,
  ...
  function getIconForSubcategory(name) {
    const icons = {
      'DVDs, CDs, & Livres': FaCompactDisc,
      'Vélos & trottinettes': FaBicycle,
      'Instruments de musique': FaDrum,
      'Sports & Fitness': MdSportsGymnastics,
      'Art & Artisanat':FaPalette,
      'Voyage & Tourisme': FaTruckPlane,
      'Autres Sports & Loisirs': FaFootball ,
    };
  ...
  function getColorForSubcategory(name) {
    const colors = {
      'DVDs, CDs, & Livres': 'bg-blue-100 text-blue-600',
      'Vélos & trottinettes': 'bg-purple-100 text-purple-600',
      'Instruments de musique': 'bg-green-100 text-green-600',
      'Sports & Fitness': 'bg-indigo-100 text-indigo-600',
      'Art & Artisanat': 'bg-orange-100 text-orange-600',
      'Voyage & Tourisme': 'bg-pink-100 text-pink-600',
      'Autres Sports & Loisirs': 'bg-yellow-100 text-yellow-600',
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 mb-8">
          ...
          <span className="text-gray-700 font-medium">{category?.name || 'Sports & Loisirs'}</span>
        </div>
        ...
        <div className="relative bg-gradient-to-r from-blue-600 to-blue-800 rounded-xl p-8 mb-12 text-white overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl mx-auto text-center">
            ...
            <h1 className="text-4xl font-bold mb-4 font-serif">{category?.name || 'Sports & Loisirs'}</h1>
            <p className="text-lg text-blue-100 mb-6">
              {category?.description || 'Découvrez notre sélection exclusive du Sports & Loisirs'}
            </p>
            ...
            <div className="relative max-w-xl mx-auto">
              <input
                type="text"
                placeholder="Rechercher des produits de Sports & Loisirs..."
        ...
        {sportsCategories.length > 0 && (
          <section className="mb-16">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-2xl font-bold text-gray-800 font-serif">Nos catégories Sports & Loisirs</h2>
              <button 
                onClick={() => navigate('/sports/toutes-categories')}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-6">
              {sportsCategories.map((category) => {
                ...
                return (
                  <div 
                    key={category.name}
                    onClick={() => navigate(`/sports/categorie/${category.name.toLowerCase()}`)}
                    className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 cursor-pointer"
                  >
        ...
        <section className="mb-16">
          <div className="flex justify-between items-center mb-8">
            ...
            <button 
              onClick={() => navigate('/sports/toutes-les-annonces')}
              className="text-blue-600 hover:text-blue-800 flex items-center"
            >
              Tout voir <ChevronRight className="h-4 w-4 ml-1" />
            </button>
