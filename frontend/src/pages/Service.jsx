...
import { FaCarSide, FaChalkboardTeacher } from 'react-icons/fa';
import { GiCookingPot, GiPlantSeed, GiBroom } from 'react-icons/gi';
...
const Service = () => {
  ...
  // Récupérer la catégorie Service et ses sous-catégories
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        const [categoryRes, propertiesRes] = await Promise.all([
          axios.get('http://localhost:8000/api/categories/categories/Service/'),
          axios.get('http://localhost:8000/api/listings/?category=Service'),
        ...
        const transformedProperties = propertiesRes.data.results.map(item => ({
          ...
          type: item.category_name ? item.subcategory.name : 'Service',
          subcategory: item.category.parent ? item.category.nameService: null,
  ...
  const serviceCategories = subcategories.map(sub => ({
    name: sub.name,
  ...
  function getIconForSubcategory(name) {
    const icons = {
      'Covoiturage': FaCarSide,
      'Cours particuliers': FaChalkboardTeacher,
      'Menage & Cuisine': GiCookingPot,
      'Travaux, bricolage et jardinage': GiPlantSeed,
      'Autres services': MdMiscellaneousServices,
    };
    return icons[name] || GiBroom;
  }

  function getColorForSubcategory(name) {
    const colors = {
      'Covoiturage': 'bg-blue-100 text-blue-600',
      'Cours particuliers': 'bg-purple-100 text-purple-600',
      'Menage & Cuisine': 'bg-green-100 text-green-600',
      'Travaux, bricolage et jardinage': 'bg-indigo-100 text-indigo-600',
      'Autres services': 'bg-pink-100 text-pink-600',
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 mb-8">
          ...
          <span className="text-gray-700 font-medium">{category?.name || 'Service'}</span>
        </div>
        ...
        <div className="relative bg-gradient-to-r from-blue-600 to-blue-800 rounded-xl p-8 mb-12 text-white overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl mx-auto text-center">
            ...
            <h1 className="text-4xl font-bold mb-4 font-serif">{category?.name || 'Service'}</h1>
            <p className="text-lg text-blue-100 mb-6">
              {category?.description || 'Découvrez nos différentes  Services'}
            </p>
        ...
        {serviceCategories.length > 0 && (
          <section className="mb-16">
            <div className="flex justify-between items-center mb-8">
              ...
              <button 
                onClick={() => navigate('/service/toutes-categories')}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-6">
              {serviceCategories.map((category) => {
                ...
                return (
                  <div 
                    key={category.name}
                    onClick={() => navigate(`/service/categorie/${category.name.toLowerCase()}`)}
                    className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 cursor-pointer"
                  >
        ...
        <section className="mb-16">
          <div className="flex justify-between items-center mb-8">
            ...
            <button 
              onClick={() => navigate('/service/toutes-les-annonces')}
              className="text-blue-600 hover:text-blue-800 flex items-center"
            >
              Tout voir <ChevronRight className="h-4 w-4 ml-1" />
            </button>
