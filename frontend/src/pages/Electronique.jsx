...
const Electronique = () => {
  ...
  // Récupérer la catégorie Electronique et ses sous-catégories
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        const [categoryRes, propertiesRes] = await Promise.all([
          axios.get('http://localhost:8000/api/categories/categories/Electronique/'),
          axios.get('http://localhost:8000/api/listings/?category=Electronique'),
        ...
        const transformedProperties = propertiesRes.data.results.map(item => ({
          ...
          type: item.category_name ? item.subcategory.name : 'Electronique',
  ...
  const electroniqueCategories = subcategories.map(sub => ({
    name: sub.name,
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 mb-8">
          ...
          <span className="text-gray-700 font-medium">{category?.name || 'Electronique'}</span>
        </div>
        ...
        <div className="relative bg-gradient-to-r from-blue-600 to-blue-800 rounded-xl p-8 mb-12 text-white overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl mx-auto text-center">
            ...
            <h1 className="text-4xl font-bold mb-4 font-serif">{category?.name || 'Electronique'}</h1>
            <p className="text-lg text-blue-100 mb-6">
              {category?.description || 'Découvrez notre sélection exclusive de Electronique'}
            </p>
            ...
            <div className="relative max-w-xl mx-auto">
              <input
                type="text"
                placeholder="Rechercher des appareil Electronique..."
        ...
        {/* Categories Grid */}
        {electroniqueCategories.length > 0 && (
          <section className="mb-16">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-2xl font-bold text-gray-800 font-serif">Nos catégories Electroniques</h2>
              <button 
                onClick={() => navigate('/electroniques/toutes-categories')}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-6">
              {electroniqueCategories.map((category) => {
                const IconComponent = category.icon;
                return (
                  <div 
                    key={category.name}
                    onClick={() => navigate(`/electronique/categorie/${category.name.toLowerCase()}`)}
                    className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 cursor-pointer"
                  >
        ...
        <section className="mb-16">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-bold text-gray-800 font-serif">
              {selectedType === "Tous" ? "Tous nos appareil electronique" : selectedType}
            </h2>
            <button 
              onClick={() => navigate('/electronique/toutes-les-annonces')}
              className="text-blue-600 hover:text-blue-800 flex items-center"
            >
              Tout voir <ChevronRight className="h-4 w-4 ml-1" />
            </button>
