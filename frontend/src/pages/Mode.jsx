...
import { 
  ...
  TabletSmartphone, SoapDispenserDroplet,  Fence, Shirt, Watch,
  Ruler, Bath,
  EyeClosed,
  Footprints,
  Eye,Briefcase
...
const Mode = () => {
  ...
  // Récupérer la catégorie Mode et ses sous-catégories
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        const [categoryRes, propertiesRes] = await Promise.all([
          axios.get('http://localhost:8000/api/categories/categories/Mode/'),
          axios.get('http://localhost:8000/api/listings/?category=Mode'),
        ...
        const transformedProperties = propertiesRes.data.results.map(item => ({
          ...
          type: item.category_name ? item.subcategory.name : 'Mode',
  ...
  const modeCategories = subcategories.map(sub => ({
    name: sub.name,
  ...
  const icons = {
    ...
    'Chaussures Homme': Footprints, // ou Shoe si dispo
    'Montres & Bijoux': Watch,
    'Cosmétiques & Parfums': SoapDispenserDroplet,
    'Chaussures Femme': Footprints,
    'Tissus & Foulards': Watch,
    'Accessoires Cheveux, Coiffure, & Cils': EyeClosed,
    'Lunettes': Eye
  };
  return icons[name] || Briefcase; // icône par défaut si non trouvée
}

  function getColorForSubcategory(name) {
    const colors = {
      'Vêtements Homme': 'bg-blue-100 text-blue-600',
      'Vêtements Femme': 'bg-purple-100 text-purple-600',
      'Chaussures Homme': 'bg-green-100 text-green-600',
      'Montres & Bijoux': 'bg-indigo-100 text-indigo-600',
      'Cosmétiques & Parfums': 'bg-orange-100 text-orange-600',
      'Chaussures Femme': 'bg-pink-100 text-pink-600',
      'Tissus & Foulards': 'bg-yellow-100 text-yellow-600',
      'Lunettes': 'bg-red-100 text-red-600'
    };
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 mb-8">
          ...
          <span className="text-gray-700 font-medium">{category?.name || 'Mode'}</span>
        </div>
        ...
        <div className="relative bg-gradient-to-r from-blue-600 to-blue-800 rounded-xl p-8 mb-12 text-white overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl mx-auto text-center">
            ...
            <h1 className="text-4xl font-bold mb-4 font-serif">{category?.name || 'Mode'}</h1>
            <p className="text-lg text-blue-100 mb-6">
              {category?.description || 'Découvrez notre sélection exclusive de la Mode'}
            </p>
        ...
        {modeCategories.length > 0 && (
          <section className="mb-16">
            <div className="flex justify-between items-center mb-8">
              ...
              <button 
                onClick={() => navigate('/mode/toutes-categories')}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-6">
              {modeCategories.map((category) => {
                ...
                return (
                  <div 
                    key={category.name}
                    onClick={() => navigate(`/mode/categorie/${category.name.toLowerCase()}`)}
                    className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 cursor-pointer"
                  >
        ...
        <section className="mb-16">
          <div className="flex justify-between items-center mb-8">
            ...
            <button 
              onClick={() => navigate('/mode/toutes-les-annonces')}
              className="text-blue-600 hover:text-blue-800 flex items-center"
            >
              Tout voir <ChevronRight className="h-4 w-4 ml-1" />
            </button>
